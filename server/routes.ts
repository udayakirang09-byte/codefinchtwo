...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Import recordings from Azure Blob Storage to database
  app.post('/api/admin/recordings/import-from-storage', authenticateSession, async (req: any, res) => {
    try {
      if (req.user.role !== 'admin') {
        return res.status(403).json({ message: 'Admin access required' });
      }

      console.log('📥 Importing recordings from Azure Blob Storage to database...');
      
      const blobs = await azureStorage.listAllBlobs();
      const recordingBlobs = blobs.filter(blob => 
        (blob.name.endsWith('.webm') || blob.name.endsWith('.mp4')) &&
        (blob.name.includes('-Final.webm') || blob.name.includes('_merged'))
      );

      console.log(`📊 Found ${recordingBlobs.length} merged recording files in Azure Storage`);

      let imported = 0;
      let skipped = 0;
      let failed = 0;

      for (const blob of recordingBlobs) {
        try {
          // Check if this recording already exists in database
          const existing = await storage.db.select()
            .from(storage.db.schema.mergedRecordings)
            .where(eq(storage.db.schema.mergedRecordings.blobPath, blob.name))
            .limit(1);
          ...
          // Extract booking info from filename if possible
          // Format: StudentName-TeacherName-Subject-DateTime-Final.webm
          let bookingId = null;
          
          // Try to find matching booking - for now, we'll create a generic entry
          await storage.db.insert(storage.db.schema.mergedRecordings).values({
            id: nanoid(),
            bookingId: bookingId,
            studentId: null,
            mentorId: null,
            blobPath: blob.name,
            blobUrl: `https://kidzaimathstore31320.blob.core.windows.net/replayknowledge/${blob.name}`,
            totalParts: 1,
            fileSizeBytes: blob.size,
            durationSeconds: null,
            mergedAt: blob.lastModified,
            expiresAt: new Date(blob.lastModified.getTime() + 30 * 24 * 60 * 60 * 1000), // 30 days retention
            aiAnalyzed: false,
            status: 'active',
          ...
          imported++;
          console.log(`✅ Imported: ${blob.name}`);
        } catch (error) {
          failed++;
          console.error(`❌ Failed to import ${blob.name}:`, error);
        }
      }
      ...
      res.json({
        ...
        skipped,
        failed,
        total: recordingBlobs.length
      });

    } catch (error) {
      console.error('❌ Error importing recordings:', error);
      res.status(500).json({ message: 'Failed to import recordings', error: String(error) });
    }
  });
