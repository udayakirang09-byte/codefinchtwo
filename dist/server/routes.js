...
import { adminConfig, teacherProfiles, courses, successStories, analyticsEvents, aiInsights, businessMetrics, complianceMonitoring, chatAnalytics, audioAnalytics, cloudDeployments, technologyStack, quantumTasks, users, bookings, systemAlerts, students } from "../shared/schema.js";
...
export async function registerRoutes(app) {
    ...
    app.get("/api/teacher/stats", async (req, res) => {
        try {
            const teacherBookings = await db.select().from(bookings).where(eq(bookings.mentorId, 'ment002'));
    ...
    app.get("/api/teacher/notifications", async (req, res) => {
        try {
            ...
            const upcomingBookings = await db.select({
                ...
                .where(and(eq(bookings.mentorId, teacherId), eq(bookings.status, 'scheduled')));
            const notifications = [];
            ...
            const completedClasses = [
                {
                    ...
                    feedbackDeadline: new Date(Date.now() + 10 * 60 * 60 * 1000),
                ...
                },
                {
                    ...
                    feedbackDeadline: new Date(Date.now() + 8 * 60 * 60 * 1000),
                    hasSubmittedFeedback: false,
                },
            ];
